...
from common import (
    calculate_global_weighted_r2, calculate_sample_weights,
...
class Stage2Dataset(Dataset):
    ...
    def __init__(self, df, transform=None, extra_features=None):
        ...
        self.df, _ = calculate_sample_weights(self.df, 'pred_species', smooth=5.0, logger=logger)
...
class Stage2ModelLog(nn.Module):
    """
    Physics-constrained model predicting only 3 base components.
    
    Direct predictions: Clover, Dead, Green
    Physics constraints: Total = C+D+G, GDM = C+G (calculated, not predicted)
    """
    def __init__(self, tab_dim, stage_index=None):
        ...
        # Only 3 prediction heads for base components
    ...
    def forward(self, img, tab):
        ...
        # Predict only base components (log space, Softplus ensures positivity)
        ...
        # Physics constraints: Calculate aggregates (NOT predicted)
    ...
    for ep in range(STAGE2_EPOCHS):
        ...
        for img, tab, y_log, y_real, w in pbar:
            ...
            with torch.amp.autocast('cuda'):
                ...
                # Weighted MSE on all 5 targets (3 predicted + 2 calculated)
        ...
        train_metrics['total_loss'] /= num_batches
        ...
        # Logging
        logger.info(
            f"E{ep+1:02d} | Train Loss: {train_metrics['total_loss']:.4f} | "
