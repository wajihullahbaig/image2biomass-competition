# unified_model.py - Dual-Head Architecture
import os
import pandas as pd
import numpy as np
import torch
...
from common import (
    BATCH_SIZE, DEVICE, NUM_EPOCHS, IMAGE_SIZE, LEARNING_RATE,
...
AUXILIARY_BACKBONE = 'tf_efficientnet_b3_ns'  # For NDVI, Height, Species, Season
BIOMASS_BACKBONE = 'swin_base_patch4_window7_224'  # For 5 biomass targets

# Available options:
# EfficientNet: tf_efficientnet_b0_ns, b1, b2, b3, b4, b5, b6, b7
# Swin: swin_tiny_patch4_window7_224, swin_small, swin_base, swin_large
...
class DualHeadDataset(Dataset):
    """
    Dataset that returns image twice (for two backbones) + tabular features + targets.
    """
    ...
    def __getitem__(self, idx):
        ...
        # Load image (same image used by both backbones)
        ...
        if self.is_train:
            ...
            return (image, tabular_data, targets, weight, species_label, 
                    season_label, state_label, ndvi, height)
        else:
...
class DualHeadBiomassPredictor(nn.Module):
    """
    Dual-head architecture with separate backbones:
    
    HEAD 1 (Auxiliary): EfficientNet → Species, Season, State, NDVI, Height
    HEAD 2 (Biomass): Swin Transformer → 5 Biomass Targets
    
    Outputs are combined using tabular features derived from Head 1.
    """
    def __init__(self, 
                 auxiliary_backbone='tf_efficientnet_b3_ns',
                 biomass_backbone='swin_base_patch4_window7_224',
        ...
        # ===== HEAD 1: AUXILIARY FEATURES (EfficientNet) =====
        self.auxiliary_backbone = timm.create_model(
            auxiliary_backbone,
            pretrained=True,
            num_classes=0
        )
        aux_features = self.auxiliary_backbone.num_features
        ...
        # ===== HEAD 2: BIOMASS PREDICTION (Swin Transformer) =====
        self.biomass_backbone = timm.create_model(
            biomass_backbone,
            pretrained=True,
            num_classes=0
        )
        biomass_features = self.biomass_backbone.num_features
        
        # Tabular feature projection (if available)
        if tabular_feature_size > 0:
            ...
            combined_features = biomass_features + 64
        else:
            self.tabular_projection = None
            combined_features = biomass_features
        
        # Biomass prediction head (5 targets)
        ...
        print(f"\n{'='*80}")
        print(f"DUAL-HEAD ARCHITECTURE INITIALIZED")
        print(f"{'='*80}")
        print(f"HEAD 1 (Auxiliary): {auxiliary_backbone}")
        print(f"  └─> Features: {aux_features}")
        print(f"  └─> Predicts: Species, Season, State, NDVI, Height")
        print(f"\nHEAD 2 (Biomass): {biomass_backbone}")
        print(f"  └─> Features: {biomass_features}")
        print(f"  └─> Tabular Features: {tabular_feature_size}")
        print(f"  └─> Predicts: 5 Biomass Targets")
        print(f"{'='*80}\n")

    def forward(self, image, tabular_data=None, return_auxiliary=False):
        """
        Forward pass through both heads.
        
        Args:
            image: Input image tensor
            tabular_data: Pre-engineered tabular features (optional)
            return_auxiliary: Whether to return auxiliary predictions
            
        Returns:
            If return_auxiliary=True: (biomass, species, season, state, ndvi, height)
            If return_auxiliary=False: biomass only
        """
        # ===== HEAD 1: Extract auxiliary predictions =====
        aux_features = self.auxiliary_backbone(image)
        
        species_logits = self.species_head(aux_features)
        season_logits = self.season_head(aux_features)
        state_logits = self.state_head(aux_features)
        ndvi_pred = self.ndvi_head(aux_features).squeeze(-1)
        height_pred = self.height_head(aux_features).squeeze(-1)
        
        # ===== HEAD 2: Biomass prediction =====
        biomass_features = self.biomass_backbone(image)
        ...
        if tabular_data is not None and self.tabular_projection is not None:
            ...
            biomass_features = torch.cat([biomass_features, tab_features], dim=1)
        
        # Predict biomass (with softplus for non-negativity in log-space)
        biomass = F.softplus(self.biomass_head(biomass_features))
        
        if return_auxiliary:
            return biomass, species_logits, season_logits, state_logits, ndvi_pred, height_pred
...
class DualHeadLoss(nn.Module):
    """
    Combined loss for dual-head architecture:
    1. Biomass loss (weighted MSE + mass balance)
    2. Auxiliary losses (species, season, state, NDVI, height)
    """
    def __init__(self, biomass_weights=None, mass_balance_alpha=0.5, aux_weight=0.15):
        super().__init__()
    ...
    def forward(self, outputs, targets, sample_weights=None, auxiliary_data=None):
        ...
        # 1. Biomass loss (weighted MSE in log-space)
        ...
        # 3. Auxiliary losses (from HEAD 1)
...
def train_dual_head_model():
    logger = setup_logging(file_name_part="dual_head_biomass_training")
    
    logger.info("="*80)
    logger.info("DUAL-HEAD BIOMASS PREDICTION MODEL")
    logger.info("="*80)
    ...
    logger.info(f"Auxiliary Backbone: {AUXILIARY_BACKBONE}")
    logger.info(f"Biomass Backbone: {BIOMASS_BACKBONE}")
    logger.info("="*80)
    ...
    # Encode categorical and add features
    train_df, val_df, encoders = encode_categorical(train_df, val_df, logger=logger)
    joblib.dump(encoders, 'dual_head_encoders.pkl')
    ...
    model = DualHeadBiomassPredictor(
        auxiliary_backbone=AUXILIARY_BACKBONE,
        biomass_backbone=BIOMASS_BACKBONE,
