...
class BiomassFeatureTransform:
    ...
    def _deterministic_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['State_Species'] = df.apply(lambda row: get_key1_specie_pair(row, key1='State'), axis=1)
        df["Season_State_Species"] = df.apply(lambda r: f"{r['Season']}_{r['State_Species']}", axis=1)
        ...
        df['Species_Season'] = df.apply(lambda row: get_key1_specie_pair(row, key1='Season', flip=True), axis=1)
