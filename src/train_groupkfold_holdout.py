...
def train_one_epoch(model, loader, optimizer, criterion_reg, criterion_species, criterion_tax, cfg, epoch, session_dir=None, logger=None,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with torch.amp.autocast('cuda'):
            ...
            if cfg.loss.use_weighted_regression_loss and official_weights_t is not None:
                ...
                if cfg.loss.reg_loss_type == 'smoothl1':
                    per_el = torch.nn.functional.smooth_l1_loss(bio_out_comp, targ_comp, reduction='none')
                else:
                    per_el = torch.nn.functional.mse_loss(bio_out_comp, targ_comp, reduction='none')
                per_target_mean = per_el.mean(dim=0)
                loss_bio_comp = (per_target_mean * comp_weights).sum() * cfg.training.biomass_feat_weight
            else:
                loss_bio_comp = criterion_reg(bio_out_comp, targ_comp) * cfg.training.biomass_feat_weight
            ...
            # Derived losses: Total and GDM in log-space
            ...
            # Standardize derived if enabled
            # Derive target totals from component targets (linear space)
            ...
            if cfg.loss.use_standardized_loss and bio_mean is not None and bio_std is not None:
                ...
                if cfg.loss.reg_loss_type == 'smoothl1':
                    loss_total = torch.nn.functional.smooth_l1_loss(derived_log_total_std, targ_total_std)
                    loss_gdm = torch.nn.functional.smooth_l1_loss(derived_log_gdm_std, targ_gdm_std)
                else:
                    loss_total = torch.nn.functional.mse_loss(derived_log_total_std, targ_total_std)
                    loss_gdm = torch.nn.functional.mse_loss(derived_log_gdm_std, targ_gdm_std)
            else:
                if cfg.loss.reg_loss_type == 'smoothl1':
                    loss_total = torch.nn.functional.smooth_l1_loss(derived_log_total, targ_total)
                    loss_gdm = torch.nn.functional.smooth_l1_loss(derived_log_gdm, targ_gdm)
                else:
                    loss_total = torch.nn.functional.mse_loss(derived_log_total, targ_total)
                    loss_gdm = torch.nn.functional.mse_loss(derived_log_gdm, targ_gdm)
        ...
        # Build full 5-dim predictions (C,D,G,Total,GDM)
        ...
        # Build full target matrix in linear space [C,D,G,Total,GDM]
        targ_c = targets_g[:, 0:1]
        targ_d = targets_g[:, 1:2]
        targ_g = targets_g[:, 2:3]
