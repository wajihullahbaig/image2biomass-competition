...
def main(args):
    # Setup
    session_dir = setup_logging(file_name_part="ts_split_train_phy")
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(df)):
        ...
        for epoch in range(EPOCHS):
            ...
            # Logging
            log_msg = f"Ep {epoch} | T_Loss: {train_metrics['train_loss']:.4f} | V_Loss: {val_metrics['val_loss']:.4f} | V_R2: {val_metrics['val_r2']:.4f} | LR: {scheduler.get_last_lr()[0]:.4f}"
            logger.info(log_msg)
